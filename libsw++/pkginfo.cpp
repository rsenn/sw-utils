...
//			db_lock lock(o_root, false);
		...
		if (o_installed_mode) {
			...
			//
			for (pkgdb::packages_t::const_iterator i = m_db->packages().begin(); i != m_db->packages().end(); ++i)
				cout << i->first << ' ' << i->second.version << endl;
		} else if (o_list_mode) {
			//
			// List package or file contents
			//
			if (m_db->find_pkg(o_arg)) {
				copy(m_db->packages()[o_arg].files.begin(), m_db->packages()[o_arg].files.end(), ostream_iterator<string>(cout, "\n"));
			} else if (file_exists(o_arg)) {
				pair<string, pkgdb::pkginfo_t> package = pkg_open(o_arg);
